...
def calcular_martes_carnaval(año):
    """Martes de Carnaval = Domingo de Pascua - 47 días"""
    a = año % 19
    b = año // 100
    c = año % 100
    d = b // 4
    ...
    return pascua - timedelta(days=47)

def es_temporada_carnaval():
    hoy    = date.today()
    inicio = date(hoy.year if hoy.month == 12 else hoy.year - 1, 12, 1)
    martes = calcular_martes_carnaval(hoy.year)
    fin    = martes + timedelta(days=7)
...
# ── RSS ────────────────────────────────────────────────────────────────────────
def get_rss_items(feeds, max_per_feed=5, total_max=3,
...
def get_efemeride():
    try:
        ...
        for item in items:
            if any(p in item.get('text', '').lower() for p in palabras_clave):
                return {'año': item.get('year', ''), 'texto': item.get('text', '')}
        best = sorted(items, key=lambda x: abs(x.get('year', 0)), reverse=True)[0]
...
PALABRAS = [
    ...
    ("Fuente sans-serif", "Tipografía sin remates, asociada a modernidad y limpieza visual, muy usada en interfaces digitales."),
    ("Above the line", "Publicidad masiva en medios tradicionales como TV, radio y prensa, dirigida a audiencias amplias."),
]
...
def render_efemeride(efem):
    ...
    return f"""
    <div style="background:#fdf6f0; border-radius:8px; padding:14px 18px;">
      <span style="font-size:0.8em; color:#e8a87c; font-weight:700;">{html_lib.escape(str(efem['año']))}</span>
...
def build_email_html(eventos, weather, news_ext, news_carnaval,
    ...
    if es_lunes and resumen_semanal:
        secciones += render_section('📋', 'Resumen de la semana', render_news_items(resumen_semanal))
...
def main():
    ...
    news_carnaval = get_rss_items(RSS_FEEDS['carnaval'], solo_espanol=True) if es_temporada_carnaval() else []
    ...
    print("📖 Efeméride y palabra del día...")
    ...
    fin_practicas     = date(2026, 4, 23)
    practicas_activas = date.today() <= fin_practicas

    if dia_en == 'Monday':
        subject = f"🫡 ¡A por el {dia_min}! Semana nueva, tú puedes 💪"
    elif dia_en == 'Tuesday':
        if practicas_activas:
            subject = f"😤 ¡A por el {dia_min}! Prácticas + clases... el día más largo, aguanta"
        else:
            subject = f"🫡 ¡A por el {dia_min}! Sin prácticas ya es otro martes 😌"
    elif dia_en == 'Wednesday':
        subject = f"🐪 ¡A por el {dia_min}! Cima de la semana, ya es cuesta abajo"
    elif dia_en == 'Thursday':
        subject = f"👀 ¡A por el {dia_min}! Mañana es viernes, aguanta un poco más"
    elif dia_en == 'Friday':
        subject = f"🎉 ¡A por el {dia_min}! ¿Te vas al pueblo o te quedas por Badajoz?"
    elif dia_en == 'Saturday':
        subject = f"😎 ¡A por el {dia_min}! Que es finde, desconecta un rato"
    elif dia_en == 'Sunday':
        subject = f"☕ ¡A por el {dia_min}! Último día, mañana vuelve el lunes..."
    else:
