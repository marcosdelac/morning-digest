...
# ── RSS ────────────────────────────────────────────────────────────────────────
def get_rss_items(feeds, max_per_feed=2, total_max=4):
    items = []
    for url in feeds:
        try:
...
def render_section(emoji, titulo, contenido_html):
    ...
    <div style="margin-bottom: 28px;">
      <h2 style="font-size: 1em; font-weight: 700; color: #1a1a1a; margin: 0 0 12px 0;
                 padding-bottom: 6px; border-bottom: 2px solid #e8a87c; display: flex;
                 align-items: center; gap: 6px;">{emoji} {titulo}</h2>
      {contenido_html}
    </div>"""
    ...
    for e in events:
        ...
        <div style="display:flex; gap:12px; margin-bottom:8px; align-items:baseline;">
          <span style="font-size:0.85em; color:#e8a87c; font-weight:700;
                min-width:60px;">{html_lib.escape(e['hora'])}</span>
...
def build_email_html(events, weather, news_ext, news_eurov, news_marketing, today_str):
    dias = {
    ...
    # Marketing: solo el primero como "lectura del día"
    lectura_html = ''
    if news_marketing:
        item = news_marketing[0]
        titulo  = html_lib.escape(item['titulo'])
        resumen = html_lib.escape(item['resumen'])
        link    = item['link']
        lectura_html = f"""
    ...
    {render_section('💡', 'Lectura del día', lectura_html)}
    """
  ...
  <div style="max-width:600px;margin:24px auto;background:#ffffff;border-radius:12px;
    ...
    <div style="background:#1a1a1a;padding:24px 32px;">
      <p style="margin:0;font-size:0.72em;letter-spacing:3px;color:#e8a87c;
                text-transform:uppercase;font-family:sans-serif;">Morning Digest · Marcos</p>
...
def main():
    ...
    today_str = f"{dia_es}, {now.day} de {mes_es} de {now.year}"
    ...
    body_html = build_email_html(events, weather, news_ext, news_eurov, news_marketing, today_str)
    subject   = f"☀️ Tu digest del {dia_es} · {now.day} de {mes_es}"
